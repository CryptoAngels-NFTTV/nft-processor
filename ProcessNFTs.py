...
class ExampleConsumer(object):
    ...
    def on_message(self, _unused_channel, basic_deliver, properties, body):
        ...
        # add to mongo
        mongo_url = "mongodb://admin:bas1cPa22@localhost:27017/"

        LOGGER.info("MONGO_URL %s : ", os.environ.get('MONGO_URL'))

        if os.environ.get('MONGO_URL') is not None:
            mongo_url = os.environ.get('MONGO_URL');  
        
        mongoClient = pymongo.MongoClient(mongo_url)
        nftCache = mongoClient["nft-cache"]
        nftCollection = nftCache["nft"]

        nftCollection.insert_one(json.loads(body))
